...
public class Matrix {
    ...
    public Matrix(int m, int n){
        if (m <= 0 || n<= 0)
            throw new IllegalArgumentException("Ошибка: Неправильные значения аргументов.");
        this.m = m;
        this.n = n;
        array = new int[m][n];
    ...
    public int swapByCol(int k){
        if (k > this.n){
            throw new IllegalArgumentException("Ошибка: Заданный номер столбца за пределом матрицы.");
        }
    ...
    public void square() throws Exception{
        if (this.m != this.n){
            throw new Exception("Ошибка: Возведение в квадрат матрицы с разными количествами строк и столбцов невозможно");
        }
